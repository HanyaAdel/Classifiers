...
class NaiveBayesClassifier():
  ...
  def __init__(self, legalLabels):
    ...
    self.initial = None  # The initial setting
  ...
  def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid): 
    ...
    self.legalLabels.
    """
    initial = dict(collections.Counter(trainingLabels))  # Get the number of training labels

    for k in initial.keys():
        initial[k] = initial[k] / float(len(trainingLabels))

    sec = dict()  # initialize a dictionary for sec

    for x, prob in initial.items():  # For every item we create a new dict
        sec[x] = collections.defaultdict(list) # Create the sec of default dictionary list

    for x, prob in initial.items():
        first = list()
        for i, ptr in enumerate(trainingLabels):        # go through the traningLabels and check the indexes and append
  ...
  def calculateLogJointProbabilities(self, datum):
    ...
    "*** YOUR CODE HERE ***"   
    for x in self.count:    
        probs = self.initial[x]  # Get the probabilty 
